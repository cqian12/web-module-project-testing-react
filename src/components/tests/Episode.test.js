...
test("renders the summary test passed as prop", ()=>{
    ...
    const summText = screen.queryByText(testEpisode.summary)

    expect(summText).toBeInTheDocument()
    expect(summText).toBeTruthy()
    expect(summText).toHaveTextContent(/new summary text/i)
});

test("renders default image when image is not defined", ()=>{
    render (<Episode episode={testEpisodeWithoutImage}/>)

    const imgTag = screen.queryAllByAltText('./stranger_things.png')

    // expect(imgTag).toBeInTheDocument()
    expect(imgTag).toBeTruthy()
    // expect(imgTag).toHaveTextContent(/new summary text/i)
})
